...
# - NEW: Mode toggle in the sidebar
...
# ---------- Unopened-pot open frequency with pair floors (prevents nonsense folds of 88 on BTN) ----------
def _preflop_open_freq(hi:str, lo:str, suited:bool, position:Position, stack_bb:float, bubble_mult:float, strict:bool) -> float:
    ...
    # Late-position bonus (kept in both modes to reflect positional advantage)
    s += {"CO": 1.0, "BTN": 1.6, "SB": 0.6}.get(position, 0.0)

    # Threshold by position/stack/bubble (bubble only in exploit mode)
    ...
    # Floors for pairs (unchanged; these act like a coarse GTO sanity guard)
    if pair:
        rv = RANK_TO_VAL[hi]
        if position == "BTN":
            f = max(f, 0.55)
...
def _preflop_three_bet_split(hi:str, lo:str, suited:bool, position:Position, vs_open_size_bb:float, stack_bb:float, strict:bool) -> Tuple[float,float,float]:
    s = chen_score(hi,lo,suited)
    ...
    if position in ("CO","BTN","SB"):
        if suited and (hi in "AKQJ" or (hi=="A" and RANK_TO_VAL[lo]<=6)):
            blf = _clamp((s - t_blf_lo)/(t_blf_hi - t_blf_lo)) * (0.55 if strict else 0.6)
        else:
            blf = _clamp((s - t_blf_lo)/(t_blf_hi - t_blf_lo)) * (0.25 if strict else 0.3)
...
def _range_advantage(prev_aggressor:str, texture:Dict[str,float], strict:bool) -> float:
    adv = 0.0
    if prev_aggressor=="hero": adv += 0.25
    adv += (0.6 - texture.get("wetness",0.4))
    adv -= 0.1 * texture.get("paired",0.0)
    # keep within same band; strict mode does NOT add exploit here
...
def gto_preflop_mix(
    hand:str, position:Position, stack_bb:float, players_left:int, total_players:int,
    facing_open:bool=False, open_size_bb:float=2.5, facing_shove:bool=False, shove_size_bb:float=0.0,
    pot_before_hero_bb:float=0.0, exploit_adj:float=0.0, strict:bool=False
) -> Dict[str,str]:
    ...
    if facing_shove and shove_size_bb>0:
        ...
        s = chen_score(hi,lo,suited)
        diff = (s/20.0) - req + (0.0 if strict else (exploit_adj*0.05))
        ...
        return {
            ...
            "all": allp,
            "explain": f"Pot-odds ~{req*100:.1f}%; Chen≈{s:.1f}. {'Strict GTO' if strict else f'Exploit {exploit_adj:+.2f}'}"
        }

    # Facing an open
    if facing_open:
        val, blf, call = _preflop_three_bet_split(hi,lo,suited,position,open_size_bb,stack_bb,strict)
        if not strict:
        ...
        return {
            ...
            "mix": mix, "all": allp,
            "explain": ("Strict GTO" if strict else f"Stack {stack_bb:.0f}bb; exploit {exploit_adj:+.2f}")
        }

    # Unopened pot (hero to act)
    ...
    return {"primary":primary,"best":best_bucket,"best_pct":best_bucket_p,"mix":mix,"all":allp,
            "explain":("Strict GTO" if strict else f"Chen; bubble x{bub:.2f}; exploit {exploit_adj:+.2f}")}
...
def gto_postflop_mix(ctx: 'PostflopContext') -> Dict[str,str]:
    ...
    adv = _range_advantage(ctx.prev_aggressor, texture, ctx.strict)
    if not ctx.strict:
        adv += ctx.exploit_adv*0.10
    multiway_penalty = max(0, ctx.n_players - 2) * 0.15
    ...
    if ctx.facing_bet_bb>0:
        ...
        return {"primary":best,"best":best,"best_pct":best_p,
                ...
                "all": allp,
                "explain":f"{ctx.street.upper()} vs {bet:.1f} into {pot:.1f} | N={ctx.n_players} | hs~{hs*100:.0f}% | adv~{adv:+.2f} | SPR~{spr:.1f} | {'Strict GTO' if ctx.strict else f'Exploit {ctx.exploit_adv:+.2f}'}"}
    ...
    return {"primary":primary,"best":best_bucket,"best_pct":best_bucket_p,"mix":mix_str,"all":allp,
            "explain":f"{ctx.street.upper()} | N={ctx.n_players} | hs~{hs*100:.0f}% | adv~{adv:+.2f} | SPR~{spr:.1f} | {'Strict GTO' if ctx.strict else f'Exploit {ctx.exploit_adv:+.2f}'}"}
...
# ================== Sidebar (includes Mode toggle) ==================
SEATS = st.sidebar.select_slider("Seats", options=[6,8,9], value=9)
mode = st.sidebar.radio("Mode", ["Strict GTO", "Exploit + ICM"], index=0, help="Strict: no personalities, no bubble factor.\nExploit + ICM: uses personalities and tournament bubble factor.")
...
for i in range(SEATS):
    with cols[i]:
        ...
        persona = st.selectbox("Personality", PERSONALITIES, index=(default_hero if i==hero_seat else default_other), key=f"p_{i}", disabled=strict_mode)
...
st.caption("Tip: Strict GTO disables personalities and bubble factor. Exploit + ICM enables them.")